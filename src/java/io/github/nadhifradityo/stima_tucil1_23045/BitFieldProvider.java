...
public class BitFieldProvider {
	protected final Class<? extends BitField> klass;

	public BitFieldProvider(Class<? extends BitField> klass) {
		this.klass = klass;
	}
	public BitFieldProvider(int width, int height, int depth) {
		this(getBestBitFieldClass(width, height, depth));
	}

	public BitField newInstance() {
		try {
